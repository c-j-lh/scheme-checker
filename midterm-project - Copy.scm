...
(define check-toplevel-form
  (lambda (v)
    (if (and (list? v) (not (null? v)) (equal? (car v) 'define))
        (if (= (length v) 3)
...
(define check-application-operands
  (lambda (args)
    (nonlazy-andmap check-expression args)))
                 ...
                 [(and (list? v) (not (null? v)))
                  (let* ([operator (car v)]
                    ...
                    (case operator ;;;;;;;
                      ...
                      [(if)
                       (if (equal? len 3)
                           (nonlazy-andmap check-expression operands)
                           (begin
                             (unless check-silently
                      ...
                      [else  ; function application
                       ...
                       (let* ([head (check-expression operator)] [rest (check-application-operands operands)])
                         (begin
                           (unless (and head check-silently)
